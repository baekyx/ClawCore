...
## 搜索策略（重要）
- 非明确要求时，最多搜索 1 次 + 抓取 2 个链接，从摘要就能回答大部分问题
- 用户说"搜10条"才扩大数量，否则默认控制在上述范围
...
class ClawCoreAgent(Agent):
    ...
    def __init__(
        ...
        # 工具调用上限 {tool_name: max_calls}, 每次 run() 重置
    ...
    def run(self, input_text: str, **kwargs) -> str:
        ...
        while current_step < self.max_steps:
            ...
            for tool_call in tool_calls:
                ...
                # 调用上限检查
                limit_msg = self._check_tool_limit(tool_name)
                if limit_msg:
                    messages.append({"role": "tool", "tool_call_id": tool_call_id, "content": limit_msg})
                    print(f"  [!] {limit_msg}")
                    continue
    ...
    def run_stream(self, input_text: str, **kwargs):
        ...
        while step < self.max_steps:
            ...
            for tc in tool_calls:
                ...
                # 调用上限检查
                limit_msg = self._check_tool_limit(tc.name)
                if limit_msg:
                    messages.append({"role": "tool", "tool_call_id": tc.id, "content": limit_msg})
                    yield StreamEvent.create(StreamEventType.TOOL_CALL_FINISH, self.name,
                        tool_name=tc.name, result=limit_msg)
                    continue
