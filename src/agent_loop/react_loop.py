"""MyClaw Agent Loop — 基于 HelloAgents ReActAgent 的扩展版
...
class MyClawAgent(Agent):
    """
    MyClaw Agent — 包装 HelloAgents ReActAgent 核心循环
    ...
    def __init__(
        ...
        config: Optional[MyClawConfig] = None,
        ...
        self.myclaw_config = config or get_config()

        # 创建 LLM
        if llm is None:
            llm = create_llm(self.myclaw_config)
        ...
        if context_pipeline is None and llm:
            context_pipeline = ContextPipeline(self.myclaw_config.agent, llm)
        self.context_pipeline = context_pipeline

        # Phase 4: Skill 管理器
        if skill_manager is None:
            skill_manager = SkillManager(self.myclaw_config.skill, llm)
        self.skill_manager = skill_manager

    def _build_helloagents_config(self):
        """将 MyClawConfig 转为 HelloAgents Config"""
        c = self.myclaw_config
